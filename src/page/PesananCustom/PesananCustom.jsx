...
function PesananCustom() {
  ...
  const hendleGetAllKategori = () => {
    axios
      .get(`http://localhost:3000/api/v1.0/categories`)
      .then((res) => {
  ...
  const handleSearchKategpri = (e) => {
    ...
    axios
      .get(
        `http://localhost:3000/api/v1.0/categories?searchName=${e.target.value}`,
      )
      .then((res) => {
        console.log(res.data.data);
        if (
          ...
          axios
            .get(
              `http://localhost:3000/api/v1.0/products?category=${e.target.id}`,
            )
            .then((res) => {
  ...
  const handleSearchProduct = (e) => {
    ...
    axios
      .get(
        `http://localhost:3000/api/v1.0/products?search=${e.target.value}&category=${kategoryId}`,
      )
      .then((res) => {
        console.log(res.data.data);
        setDataProduct(res.data.data);
  ...
  const hendleClickKategori = (e) => {
    ...
    axios
      .get(`http://localhost:3000/api/v1.0/products?category=${e.target.id}`)
      .then((res) => {
        setDataProduct(res.data.data);
      })
      .catch((err) => {
  ...
  const hendleTambahOrder = (name, Deskripsi_produk) => {
    ...
    if (itemIndexTambah >= 0) {
      ...
      newArrayDataOrder[itemIndexTambah].jumlahHarga +=
        newArrayDataOrder[itemIndexTambah].harga;
            ...
            <div className={style.itemContentOrderCustom}>
              ...
              <div className={style.inputWrapper}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    `${setNamaKategori(e.target.value)} ${handleSearchKategpri(
                      e,
                    )}`
                  }
                />
                  ...
                  dataKategory.map((data, key) => {
                    return (
                      <div
                        id={data.id}
                        key={key}
                        onClick={(e) => hendleClickKategori(e)}
