...
function OrderAdmin() {
    ...
    const [messageErrorPersediaanBahanBaku, setMessageErrorPersediaanBahanBaku] = useState([])
    ...
    const hendleStatusPesanan = (id_Product_order, status, event) => {
        ...
        const index = dataCkeckBoxStatusPesanan?.findIndex(data => data.id === id_Product_order)
        ...
        axios.patch(`http://localhost:3000/api/v1.0/productOrder/${id_Product_order}`,{status: status}, { headers: { Authorization: `Bearer ${dataLogin.dataLogin.token}`}})
        .then((res) => {
            console.log(res.data.data)
            hendleGetByIdProductOrder(res.data.data?.id_orders, res.data.data?.id_user)
        }).catch((err) => {
            console.log(err)
            if (err.response.data.message.length !== 0) {
                alert(`Bahan Baku Tidak Mencukupi, ${err.response.data.message.map(data => ` ${data.bahanBaku.nama} Tersisa ${data.jumlah} ${data.satuan}`)}`)
    ...
    const hendleGetByIdProductOrder = (id_order, id_user) => {
        ...
        axios.get(`http://localhost:3000/api/v1.0/productOrder/customers?order_id=${id_order}&id_user=${id_user}`, { headers: { Authorization: `Bearer ${dataLogin.dataLogin.token}`}})
        .then((res) => {
            console.log(res.data.data)
            ...
            res.data.data.map(objectt => {
                setDataCheckBoxStatusPesanan(data => [...data, {id: objectt.id, status: objectt.status}])
            })
    ...
    const hendleTransaksi = (idOrder, statusTransaksi) => {
        ...
        axios.patch(`http://localhost:3000/api/v1.0/orders/${idOrder}`, {statusTransaksi: statusTransaksi})
        .then((res) => {
            console.log(res.data.data)
        }).catch((err) => {
            console.log(err)
        ...
        axios.post(`http://localhost:3000/api/v1.0/transaksi`, data)
        .then((res) => {
            console.log(res.data.data)
        }).catch((err) => {
            console.log(err)
                                    ...
                                    return (
                                        <tr key={key}>
                                            ...
                                            <td  className={style.options}>
                                                ...
                                                <span ref={buttonRef} className={`material-symbols-outlined ${style.iconOptions}`} onClick={() => hendleGetByIdProductOrder(data.id, data.users.id)}>visibility</span>
                                            </td>
                                        </tr>
                                    )
                                })
                ...
                {statusOrderProduct === "Product_Order" && (
                    <div className={style.containerPopUpStatus}>
                        ...
                        <div className={style.top}>
                            <span onClick={() => `${setStatusOrderProduct("")} ${hendleClose()}`} className="material-symbols-outlined">close</span>
                        </div>
                                         ...
                                         <div key={key} className={style.content}>
                                            ...
                                            <div className={style.contentCenter}>
                                                <div className={style.contentCenterItem}>
                                                    ...
                                                    <span className={style.subData}><span style={{fontWeight: "bold", fontSize: "14px"}}>Kategori : </span>{data?.products.categories.name}</span></span>
                                                    <span>
                                                        <span style={{fontWeight: "bold", fontSize: "14px"}}>Jumlah : </span>
                                                            {data.jumlah}
                                                            {data?.products?.name?.toLowerCase() == "kitcen set" ? " / meter jalan" : ""}
                                                            {data?.products?.name?.toLowerCase() == "set kamar tidur" ? " / meter jalan" : ""}
                                                            {data?.products?.name?.toLowerCase() == "backdrop / partisi ruangan / mini bar" ? " / meter jalan" : ""}
                                                        </span>
                                                    <span><p  style={{fontWeight: "bold", fontSize: "14px"}}>Deskripsi Produk : </p>{data.products.Deskripsi_produk}</span>
                                                </div>
                                                ...
                                                <div className={style.contentCenterItem}>
                                                    <span className={style.cardHarga}>{new Intl.NumberFormat("id-ID", {style: "currency", currency: "IDR"}).format(data.Price).replace(/(\.|,)00$/g, "")}</span>
                                                    <div className={style.average} >
                                                        {(isPembayaranDP == true || isPembayaranLunas == true || (isPembayaranDP == true && isPembayaranLunas == true)) ? (
                                                            <>
                                                                <span>{dataCkeckBoxStatusPesanan[key]?.status == 2 ? "Pesanan Diterima" : "Pesanan Ditolak"}</span>
                                                            </>
                                                        ) :
                                                            <>
                                                                <div>
                                                                    <input id={`${data.id} trima`} type="checkbox" checked={dataCkeckBoxStatusPesanan[key]?.status == 2 ? true : ""}  onChange={(e) => hendleStatusPesanan(data.id, 2, e)}/>
                                                                    <label htmlFor={`${data.id} trima`}>Pesanan diterima</label>
                                                                </div>
                                                                <div>
                                                                    <input id={`${data.id} tolak`} type="checkbox" checked={dataCkeckBoxStatusPesanan[key]?.status == 1 ? true : ""} onChange={(e) => hendleStatusPesanan(data.id, 1, e)}/>
                        ...
                        {console.log(dataBuktiBayar)}
                            {dataBuktiBayar.buktiBayar?.length !== 0 && (
                                <div className={style.formOrder}>
                                                        ...
                                                        <div className={style.contentItem}>
                                                            ...
                                                            <div className={style.item}>
                                                                {isPembayaranLunas === true ?
                                                                    <p>Terverivikasi</p>
                                                                    :
                                                                    <input type="checkbox" checked={isPembayaranLunas == true ? true : false} onClick={() => hendleTransaksi(dataBuktiBayar.id, data.statusTransaksi)} />
                                                                }
                                                            </div>
                                                        </div>
                                                    :
                                                        <div className={style.contentItem}>
                                                            ...
                                                            <div className={style.item}>
                                                                {isPembayaranDP === true ?
                                                                    <p>Terverivikasi</p>
                                                                    :
                                                                    <input type="checkbox" checked={isPembayaranDP == true ? true : false} onClick={() => hendleTransaksi(dataBuktiBayar.id, data.statusTransaksi)} />
                                                                }
                                                            </div>
                                                        </div>
                                                    }
                                    ...
                                    <div className={style.kesesuaianBuktiBayar}>
