...
export const dataOrder = (state= initialstate , action) => {
    switch (action.type) {
        case 'SAVE_ORDER':
            const itemIndexTambah = state.dataOrder.findIndex((item) => item.id === action.payload.id)
        ...
        case 'KURANG_ORDER':
            const dataKurang = state.dataOrder.filter((item) => item.id === action.payload.id)
            const itemIndexKurang = state.dataOrder.findIndex((item) => item.id === action.payload.id)
            ...
            if (dataKurang[0].jumlah > 1) {
                ...
                return {
                    ...state,
                    dataOrder: state.dataOrder.filter((data) => data.id !== action.payload.id)
                };
            }
        case 'HAPUS_ORDER':
            return {
