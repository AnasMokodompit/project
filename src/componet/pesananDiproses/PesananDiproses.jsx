...
function PesananDiproses() {
    ...
    const hendleOrderByCustemers = () => {
        if (!!isLogin) {
          ...
          axios
            .get(`http://localhost:3000/api/v1.0/productOrder/customers?statusKategori=Pesanan Diproses`, {
              headers: { Authorization: `Bearer ${dataLogin?.dataLogin?.token}` },
            })
            .then((res) => {
              console.log(res.data.data);
                ...
                {dataProduct.length !== 0 &&
                    ...
                    return (
                        <div
                            ...
                            <span>Status : <span style={{color: "red"}}>{data.pesan_status}</span></span>
                            <span>Selesai : <span  id={data.id}>{formatData(data.id, data.tanggal_pengerjaan)}</span></span>
                        </div>
                    );
                    })}
                </div>
